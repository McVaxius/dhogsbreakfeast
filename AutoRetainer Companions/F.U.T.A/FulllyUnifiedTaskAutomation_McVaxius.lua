...
--EXCLUSION LIST END--
IPC.Automaton.SetTweakState("ARQuesting", true)
...
FUTA_defaults = {
    {
        ...
        {"FISH", 0},								--Y--{}[i][2][1..2]--level, -1 = dont do anything, 0 = scan me for fish levels, 100 = dont do anything, 101 = automatically pick this char everytime (default indexed char), minimum = pick this char if no 101 exists (default indexed char)
...
if current and current > -1 or current == nil then
    ...
    yield("/wait 0.5")
	--fcexist = Addons.GetAddon("FreeCompanyStatus"):GetNode(1, 4, 52, 57).Text or 0 --57 always seems to have the text O_o
fcexist = Addons.GetAddon("FreeCompanyStatus"):GetNode(1, 4, 52, 56).Text or "Housing not available."
	if fcexist == "Housing not available." then
		yield("/echo We don't have a FC house. skipping fc house data extraction.")
	end
	if string.len(fcexist) > 0 then
